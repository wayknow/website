...
| ClearJSON product | ✅ | v1.1.4: features, MCP server section (10 tools, `npx -y clearjson-mcp`), comparison table (incl. JSON Pretty Pro), FAQ. CWS + Edge store links, both stores live. Glama score A/A/B. npm package published. **2026-08-15: 商业化改造中——买断 $29 → 免费核心 + Pro 订阅（$2.99/月 或 $19.99/年，Coming in 2.0: JSON diff/JSONPath/YAML·XML 转换），落地页定价区块已改，waitlist 收集启动（验证期：≥50 waitlist 或 ≥5 付费 = 方向成立）** |
...
| Blog: CSSPick CSS Inspector | ✅ | Competitive comparison → CSSPick. CSS Scan $120 list ($69 promo) price critique, DevTools slowness, 7 groups, $19 lifetime. Price claims verified against getcssscan.com 2026-08-13 (was incorrectly $95). 2026-08-11. `blog/csspick-css-inspector.html` |
